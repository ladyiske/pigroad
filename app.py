...
st.markdown("""
    ...
    /* 돼지 오른쪽 버튼 스타일 */
    .right-btn-area { display: flex; align-items: center; justify-content: center; height: 100%; padding-top: 50px; }
    </style>
...
if st.session_state.clicked and st.session_state.recommended_menu:
    ...
    with col_btn:
        st.markdown('<div class="right-btn-area">', unsafe_allow_html=True)
        if st.button("📋 결과 복사"):
    ...
    # 선택 영역
    category = st.selectbox("카테고리를 선택하세요", ["한식", "중식", "양식", "일식", "동남아", "디저트"])
    if st.button("주문하기! 🛎️"):
